...
def ingest_permits(
    ...
    for permit in permits:
        ...
        inspections_raw = permit.get("inspections")
        inspections_json = json.dumps(inspections_raw) if inspections_raw is not None else None
        payload = {
            ...
            "raw_applicant_company": permit.get("raw_applicant_company"),
            "raw_applicant_address": permit.get("raw_applicant_address"),
            "raw_applicant_phone": permit.get("raw_applicant_phone"),
            "raw_applicant_email": permit.get("raw_applicant_email"),
            "raw_contractor_license_number": permit.get("raw_contractor_license_number"),
            "raw_contractor_license_type": permit.get("raw_contractor_license_type"),
            "raw_additional_licensed_professionals": permit.get("raw_additional_licensed_professionals"),
        ...
        cur.execute(
            ...
            SELECT id, subdivision_id, builder_id, address, parcel_id,
                   ...
                   raw_applicant_company, raw_applicant_address, raw_applicant_phone,
                   raw_applicant_email, raw_contractor_license_number,
                   raw_contractor_license_type,
                   raw_additional_licensed_professionals,
                   latitude, longitude,
                   inspections_json
            FROM pt_permits
        ...
        if existing is None:
            cur.execute(
                ...
                (
                    ...
                    payload["raw_applicant_company"],
                    payload["raw_applicant_address"],
                    payload["raw_applicant_phone"],
                    payload["raw_applicant_email"],
                    payload["raw_contractor_license_number"],
                    payload["raw_contractor_license_type"],
                    payload["raw_additional_licensed_professionals"],
                    payload["latitude"],
                    payload["longitude"],
        ...
        existing_dict = {
            ...
            "raw_applicant_company": existing[15],
            "raw_applicant_address": existing[16],
            "raw_applicant_phone": existing[17],
            "raw_applicant_email": existing[18],
            "raw_contractor_license_number": existing[19],
            "raw_contractor_license_type": existing[20],
            "raw_additional_licensed_professionals": existing[21],
            "latitude": existing[22],
            "longitude": existing[23],
            "inspections_json": json.dumps(existing[24]) if isinstance(existing[24], (list, dict)) else existing[24],
        }
        ...
        cur.execute(
            ...
            SET subdivision_id = %s, builder_id = %s, address = %s, parcel_id = %s,
                ...
                raw_applicant_company = %s, raw_applicant_address = %s, raw_applicant_phone = %s,
                raw_applicant_email = %s, raw_contractor_license_number = %s,
                raw_contractor_license_type = %s,
                raw_additional_licensed_professionals = %s,
...
def get_permits_payload(conn, filters: dict) -> dict:
    ...
    cur.execute(
        ...
        SELECT
            ...
            p.raw_owner_name,
            p.raw_owner_address,
            p.raw_applicant_company,
            p.raw_applicant_address,
            p.raw_applicant_phone,
            p.raw_applicant_email,
            p.raw_contractor_license_number,
            p.raw_contractor_license_type,
            p.raw_additional_licensed_professionals,
            p.inspections_json
        {base_sql}
        ORDER BY {sort_sql}
        LIMIT %s OFFSET %s
