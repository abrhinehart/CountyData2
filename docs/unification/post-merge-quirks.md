...
- **Status**: `cleared` (Bay County schema adaptation 2026-04-10; `pt_scraper_artifacts` cold-path closed 2026-04-11 via Panama City Beach validation triad)
...
- `modules/permits/services.py:2091-2148` (`_persist_adapter_trace_artifacts`) — **validated 2026-04-11** — exercised live for the first time under the unified schema via Panama City Beach (id=3, iWorq adapter). Single `_run_single_adapter` call on the default 120-day bootstrap window (2025-12-12 → 2026-04-11) ingested 46 residential permits and persisted **114 trace artifacts** to `pt_scraper_artifacts` (1 `search-results` + 67 `search-pagination` + 46 `detail-page`). All 10 schema columns populated 114/114: `jurisdiction_id`, `adapter_slug`, `scrape_run_id`, `artifact_type`, `method`, `url`, `status_code`, `content_type`, `excerpt_text`, `metadata_json`, `created_at`. `metadata_json` correctly serializes the per-trace metadata dicts (e.g. `{"page": 1, "end_date": "2026-04-11", "start_date": "2025-12-12"}` for search-results, `{"detail_url": "https://..."}` for detail-page). Schema matches migration 015 exactly. URL redaction is applied (query strings replaced with `[redacted]`). Excerpt text is HTML-stripped via `_build_trace_excerpt`. This closes the PT Phase 3 cold-path acceptance surface flagged by the original Entry 4 note. Execution log preserved at `pt_record_trace.log`. **Second `record_trace`-emitting adapter family also validated 2026-04-11** via Polk County (id=4, Accela ASP.NET ViewState postback flow, see new Polk bullet below) — both iWorq and Accela portal families now have strict-pass evidence on the `_persist_adapter_trace_artifacts` cold path.
- `modules/permits/scrapers/base.py:159-181` (`record_response_trace`) and `base.py:123-157` (`get_cached_text(..., artifact_type=...)`) — **validated 2026-04-11** — both adapter-side trace-capture entry points exercised via Panama City Beach. `record_response_trace` fired 68 times (1 search-results + 67 search-pagination), `get_cached_text(artifact_type="detail-page")` fired 46 times. `consume_trace_artifacts()` handoff to `_persist_adapter_trace_artifacts` confirmed lossless (68 + 46 = 114, matches the post-flight row count exactly). `_redact_url`, `_truncate_text`, and `_build_trace_excerpt` all worked correctly on the live HTML responses. Polk County 2026-04-11 run added 97 more trace emissions (3 `search-results` + 9 `search-pagination` + 85 `detail-page`), exercising the shared `record_response_trace` entry point from a second adapter family. Total `pt_scraper_artifacts` rows post-Polk: 211. Notably, the Polk run exercised the **POST method** path of `record_response_trace` (Accela uses ASP.NET form posts for both search and pagination, vs PCB's all-GET iWorq flow) and the lowercase `text/html; charset=utf-8` content-type variant (vs PCB's uppercase `UTF-8`), proving the column accepts arbitrary case in content-type strings.
- `modules/permits/scrapers/adapters/panama_city_beach.py:98,154` — **validated 2026-04-11** — both PCB adapter trace-call sites fired against the live iWorq portal. 46 distinct permits parsed across 67 search pagination requests; every permit's detail page was fetched, cached, and traced.
- `modules/permits/scrapers/adapters/polk_county.py:113,139,200` — **validated 2026-04-11** — all three Polk adapter trace-call sites fired against the live Accela ACA portal (`https://aca-prod.accela.com/POLKCO/`). `record_response_trace("search-results", ...)` at line 113 fired 3 times (once per `_fetch_range` recursion leaf — confirms the range-split-on-100-cap recursion works under unified schema); `record_response_trace("search-pagination", ...)` at line 139 fired 9 times across the `_post_back` loop with `__doPostBack` event-target navigation; `get_cached_text(artifact_type="detail-page", ...)` at line 200 fired 85 times (one per unique permit detail URL, deduped via `_request_cache`). Total 97 artifacts persisted to `pt_scraper_artifacts` under slug `polk-county`, all 10 schema columns populated 97/97. This validates the ASP.NET ViewState postback flow as the second `record_trace`-emitting adapter family (after iWorq/PCB). `extract_form_fields` (base.py:98-121) also exercised live as the shared ViewState-field extractor — the Polk run is the first live exercise of that helper. Window: 2026-03-28..2026-04-11 (14 days, narrowed from the adapter's 120-day default after the first attempt's 120-day window blew the in-memory trace buffer past 350MB and was killed at 13 minutes — the 14-day rolling-overlap window is what production would use after bootstrap and exercises the exact same code path with bounded resource usage). Execution log preserved at `pt_polk_county.log`. Per-type metadata sample: search-results = `{"end_date": "2026-04-11", "start_date": "2026-03-28", "record_type": "Building/Residential/New/NA"}`, search-pagination = `{"event_target": "ctl00$PlaceHolderMain$dgvPermitList$gdvPermitList$ctl13$ctl06"}`, detail-page = `{"detail_href": "https://aca-prod.accela.com/POLKCO/Cap/CapDetail.aspx?...&capID1=REC26&..."}`.

**Fix pattern**
No schema-adaptation fix needed. Reference for future porters: raw-SQL writers to shared spine tables need the same `county TEXT NOT NULL` awareness as ORM writers; PT handled it via `(SELECT name FROM counties WHERE id = %s)` subquery. The `pt_scraper_artifacts` table was not touched by the original 2026-04-10 Bay County run because `BayCountyAdapter` does not call `record_trace`. **Cold-path validation completed 2026-04-11 via Panama City Beach** (iWorq) **and Polk County** (Accela), confirming both `record_trace`-emitting adapter families clear the `pt_scraper_artifacts` write path under the unified schema. **Planner correction (2026-04-11)**: the original note named "Lynn Haven, Callaway, Panama City Beach, Panama City" as Cloudpermit-family adapters that call `record_trace`. Of that list, Lynn Haven and Callaway do not exist as adapters, and Panama City (the only true Cloudpermit adapter) uses a direct JSON API and does NOT call `record_trace` either. The real `record_trace` callers as of 2026-04-11 are `panama_city_beach.py` (iWorq) and `polk_county.py` (Accela), verified via `grep`. Both have now been validated; Entry 4's PT Phase 3 cold-path acceptance surface is fully closed. For the builder-dedup content-logic bug surfaced by the original Bay County run, see Entry 5.

The Panama City Beach 2026-04-11 run also live-validated several adjacent code paths: (a) `_resolve_subdivision_id` matched 9 of 46 permits to existing Bay County subdivisions via geometry/address lookup with no NULL-county insertions (Entry 1 regression guard clean: pre and post NULL count = 0); (b) `_ensure_builder_id` correctly resolved 8 distinct builders including DR Horton (25 permits), Carnley Construction (4), Harris Doyle Homes (4), and others, with no new canonical-name duplicates introduced (Entry 5 regression guard clean: pre and post duplicate count = 0); (c) `geocode_missing_permits` queried 45 addresses, matched 20, updated 21, and identified 9 subdivision matches; (d) `pt_permits` table now contains 46 PCB rows alongside the 82 Bay County rows from the original validation, with full subdivision/builder join coverage.

The Polk County 2026-04-11 run added 85 more permits to `pt_permits` and 97 trace artifacts to `pt_scraper_artifacts` (totals after Polk: 213 PT permits across 3 jurisdictions, 211 trace artifacts across 2 adapter families). Adjacent code-path validation: (a) `_resolve_subdivision_id` matched 1 of 85 Polk permits to an existing subdivision via geometry/address lookup, with no NULL-county insertions (Entry 1 regression guard clean: pre=0, post=0); (b) `_ensure_builder_id` resolved Polk's contractor strings cleanly with DR Horton again surfacing as a recognizable national builder (11 permits) — note that Polk's Accela `_fetch_detail_fields` populates `licensed_professional` as the contractor field, which yields very long subcontractor names for HVAC/plumbing subs (e.g. the top "builder" by Polk permit count is a 200-char string starting with "Robert C Blankenship Bayonet Plumbing..." — this is data-quality color, NOT a validation bug, the builders.canonical_name column accepts long strings without truncation); (c) `geocode_missing_permits` queried 85 addresses, matched 20, updated 20, and identified 1 subdivision match; (d) range-split recursion at `_fetch_range` (line 88, 100-permit cap) fired and recursed 3 leaves, proving the recursive search-window split works under unified schema. **Window-narrowing note**: the Polk default 120-day bootstrap window was attempted first and aborted at 13 minutes / 355MB in-memory trace buffer (estimated >800 artifacts pending persistence, well above the driver's 200-artifact runaway cap). The triad narrowed the window to 14 days (Polk's `rolling_overlap_days`, the value production would use after bootstrap) — same code path, bounded resource usage. The kill+narrow decision is documented in `tmp/run_pt_polk_county.py`'s `WINDOW_OVERRIDE_DAYS` constant and inline comments for the next porter.
...
- **Status**: `fixed` (in-module fix landed 2026-04-11 in the Entry 11 fix triad; live offline-probe evidence stands from both the drift case (Mar 9 agenda, `source_doc.id=8`) and the regression-guard case (Feb 9 agenda, `source_doc.id=9`))
...
**Seen in**
- `modules/commission/packet_fetcher.py:146-193` — **fixed 2026-04-11** — `_parse_desc_fields` now handles three shapes: (1) legacy sibling-span after `<strong>`, (2) second-span-inside-`<p>` fallback, and (3) bare NavigableString after `</strong>`, read via `p_tag.get_text()[idx + len(strong_text):].lstrip(": \xa0").strip()` where `idx` is the position of the strong's own text inside the full `<p>` text. The char-set `": \xa0"` catches all leading-separator variants seen in the wild (`<strong>Label</strong>: value`, `<strong>Label:&nbsp;</strong>value`, and combinations). Fix applied 2026-04-11 in the Entry 11 fix triad. Offline probes against the preserved `cr_source_documents` rows confirm both branches: row `id=8` (Mar 9 agenda, cases `CPC-PLN-2026-0626` and `CPC-PLN-2026-0633`) now returns **8 keys per case** (`acreage (+/-)`, `address/location`, `applicant`, `application type`, `city commission public hearing date (s)`, `owner`, `planning board public hearing date`, `request`), up from 0 pre-fix; row `id=9` (Feb 9 agenda, case `CPC-PLN-2026-0176`) still returns the same 7 keys as before, confirming the span-wrapped happy-path is unchanged. Unit tests live at `tests/test_packet_fetcher.py` with 9 cases covering span-wrapped, three bare-text variants (including the full Mar 9 three-paragraph fixture), mixed-shape, empty-desc, no-strong, strong-without-value, and strong-with-whitespace-only-value edges. The `tmp/run_cr_civicplus_packet.py` driver and `cr_civicplus_packet.log` from the 2026-04-11 validation triad remain as the original surfacing evidence; they were not re-run (see Follow-up note below).
...
**Fixed in this triad (historical context)**
The 2026-04-11 civicplus validation triad surfaced this drift but deferred the fix to preserve validation-triad discipline. The follow-up Entry 11 fix triad (also 2026-04-11) applied the in-module fix plus the `tests/test_packet_fetcher.py` unit suite. The Mar 9 agenda's two affected actions (`SweetBay Town Center`, `230 McKENZIE AVE`) had already been populated with `address`, `applicant_name`, `acreage`, and `parcel_ids` by the LLM extractor during the validation run, so the immediate data-quality impact of this drift was bounded — packet_fetcher's role is fallback enrichment, and the LLM was thorough enough to make that fallback unnecessary on the particular agenda that first surfaced the drift. The fix still matters for future agendas where the LLM leaves gaps that packet_fetcher would normally fill in the CivicPlus branch.

**Follow-up note**
The Entry 11 fix triad validated the fix via two mechanisms rather than a live driver re-run: (a) 9 inline-fixture unit tests at `tests/test_packet_fetcher.py` covering span-wrapped, three bare-text variants, mixed shape, and four edge cases; (b) offline probes that called `_parse_desc_fields` directly against the preserved `cr_source_documents` rows `id=8` and `id=9`, bypassing the merge loop and its LLM-pre-populated short-circuit. The live `tmp/run_cr_civicplus_packet.py` driver was NOT re-run because the merge loop's `enriched_count` would still return 0 against the already-populated action rows — the offline probe is the correct acceptance criterion for this fix class (parser-correctness validation against preserved source HTML). Pattern lesson: when a fallback enricher's effect is masked by upstream pre-population, test the enricher in isolation rather than re-running the full pipeline.
