...
- **Permit / utility / licensing portal posture:** **NO ONLINE PERMIT SYSTEM observed.** The `/building-department` page (51,369 bytes) does not link out to BS&A / Accela / iWorQ / GovBuilt / CivicPlus portal / SmartGov / CityView / EnerGov / Cloudpermit / PermitTrax. The only non-city outbound is the CivicPlus.com footer link. Mulberry is paper/email like Bartow — permit workflow surfaces only as CMS text pages, not as a queryable data surface.
...
**Totals:** ~24 HTTPS requests, 0 × 429, 0 × captcha; 5 APIs documented; 6 scrape targets; 0 external platforms cross-referenced (Municode not characterized); 6+ dead/placeholder tenants documented. **No new platforms** (Drupal ACSF row added in Eagle Lake map).
