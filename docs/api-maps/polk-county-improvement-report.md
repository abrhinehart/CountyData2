...
|----|----------|---------|----------------|--------------------|--------|------|----------|
| ACCELA-01 | Coverage Gap | ✅ **Shipped** — `AccelaCitizenAccessAdapter` now iterates `target_record_types: tuple[str, ...]`; Polk overrides with a curated 9-type subset covering Residential/New, Residential/Renovation, Residential/Accessory, Commercial/New, Trades/Re-Roof, Trades/Electrical, Trades/Plumbing, Trades/Mechanical, Trades/Pool.  `record_type_delay=2.0s` between iterations; per-type try/except; results dedupe by permit_number via outer `permits_by_number`.  Siblings (Citrus / Lake Alfred / Winter Haven) keep the single-type default. | 4 residential + 9 single-trade + 6 commercial + mobile-home + Land Dev (27 record types in POLKCO Building module per April 2026 recon, not 34 as originally estimated) | `target_record_types` tuple on `PolkCountyAdapter`; `_resolve_record_types()` on the base adapter falls back to `(target_record_type,)` when the tuple is empty.  Remaining Polk types intentionally out of scope: sign permits, mobile-home variants, demolition, contractor-licensing renewals, fence/wall, gas, window/door, accessory MH skirting, search-request / pre-permit / admin / temporary-use. | M | Low | ✅ Shipped |
...
| ACCELA-04 | Field Mapping | ✅ **Shipped** — Applicant company/address/phone/email + contractor license type/number captured via regex on the flattened CapDetail text (live-validated on Polk BR-2026-2894: `Jeff Cunningham LGI Homes` / `8137315791` / `jeff.cunningham@lgihomes.com` / `CAC1820196` / `Air Condition Class A`). Migration `024_permit_contact_fields.sql` adds six TEXT columns. Ingestion coverage 91-100% on Polk (298 permits, 30-day window). Subcontractor list follow-up shipped as ACCELA-04a. | Full name, company, license type+number, phone, email, fax, mailing address; "View Additional Licensed Professionals" subcontractor list | `applicant_company_pattern` / `applicant_address_pattern` / `applicant_phone_pattern` / `applicant_email_pattern` / `contractor_license_number_pattern` / `contractor_license_type_pattern` in `accela_citizen_access.py`. REST `/contacts` + `/professionals` still token-gated (see ACCELA-02). | S | Low | ✅ Shipped |
| ACCELA-04a | Field Mapping | ✅ **Shipped** — Subcontractor "View Additional Licensed Professionals>>" list captured via `additional_licensed_professionals_pattern` + `_parse_additional_lps()` helper in `accela_citizen_access.py`.  Serialized as `NAME|LICENSE_NUMBER|LICENSE_TYPE` per LP, joined by `; ` between LPs.  Migration `025_permit_subcontractors.sql` adds one TEXT column (`raw_additional_licensed_professionals`).  Live-recon on Polk BR-2026-2659 captured 4 subs (Roofing CCC1336147, Plumbing CFC1431566, General CGC1526755, Electric With Alarm EC13015339). | "View Additional Licensed Professionals>>" enumerated list on CapDetail flat text; JS DOM toggle (`DisplayAdditionInfo`) means data is already inline. | Regex terminated on `Project Description:`/`Owner:`/`More Details` to avoid bleeding into the Application Information section.  Per-LP parse extracts name (leading tokens until email), license number (trailing FL contractor code), and type (words between ZIP and license number). | S | Low | ✅ Shipped |
...
| ACCELA-11 | Field Mapping | ✅ **Shipped / verified already wired** — `_run_single_adapter` in `modules/permits/services.py:1795-1804` already computes `missing_coordinate_addresses` from the adapter output and calls `geocode_missing_permits` post-ingest.  `modules/permits/geocoding.py` has `", FL"` hints for Polk County / Lake Alfred / Winter Haven + Davenport/Haines City/Lake Hamilton/Bay County/Panama City/Panama City Beach.  April 2026 smoke: batch-geocode of three Polk Accela addresses matched 2/3 (known-rural `7118 CANOPY LN LAKELAND` misses due to Census-TIGER rural-road gaps, not a wiring bug; `410 E ORANGE ST LAKELAND` → `(28.0418, -81.9540)`, `600 NORTH BROADWAY AVE BARTOW` → `(27.9006, -81.8433)`). | REST `/v4/geo/geocode/reverse`, or `xCoordinate`/`yCoordinate` on address object | Ticket closed on verification — no code change needed.  REST `/geo/geocode/reverse` remains blocked by ACCELA-02; `geocoding.py` continues to be the actionable route.  Downstream mitigation for rural-road misses is out of scope here (consider adding fallback to ArcGIS BI Polk centroid in a separate ticket if coverage gaps matter for analytics). | S | Low | ✅ Shipped |
...
| ACCELA-14 | Robustness | ✅ **Shipped** — `scripts/drift_canary.py` runs three checks: (1) `audit_api_maps.py --fail-on-drift` subprocess, (2) live Polk 30-day fetch anchored on BR-2026-2894 (with fallback to most-recent owner-bearing permit), (3) NotNull asserts on `raw_owner_name`, `raw_applicant_company`, `raw_contractor_license_number`, `parcel_id`, `valuation`.  Pass writes `docs/sessions/drift-canary-<date>.md`; fail writes `DRIFT_CANARY_FAILED_<date>.md` at repo root and exits 2.  Scheduled monthly via Windows Task Scheduler (`schtasks /Create /SC MONTHLY /D 1 /ST 09:00 /TN "CountyData2-DriftCanary" ...`).  Runbook at `docs/permits/drift-canary-runbook.md` documents manual-fallback workflow. | Deterministic JSON via REST would have removed this risk, but REST is blocked (ACCELA-02); HTML-side canary is the only mitigation. | See `scripts/drift_canary.py` + `docs/permits/drift-canary-runbook.md`.  Permanent guardrail since the "until ACCELA-02 lands" escape hatch is gone. | S | Low | ✅ Shipped |
...
1. **P0**: ACCELA-05 — close the inspections drift bug (1-2 hours). _[shipped in commit `be5d02f`.]_
2. **P1 / strategic pivot**: ACCELA-03, ACCELA-04, ACCELA-04a — all shipped; see above.
3. **P1 / quick wins**: ACCELA-11 ✅ shipped (verified wiring + smoke), ACCELA-14 ✅ shipped (`scripts/drift_canary.py` + Task Scheduler registration).
4. **P1 / expansion**: ACCELA-01 ✅ shipped (9-type Polk curated subset; siblings keep single-type).  ACCELA-16 partial (Enforcement-module HTML probe for Winter Haven) remains open.
...
| ACCELA-04 | Accela | Field Mapping | ✅ Shipped | S | Low | Structured contact parsing — 6 new raw_* columns via migration 024 |
